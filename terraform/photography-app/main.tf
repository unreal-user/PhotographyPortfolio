...
# =============================================
# Thumbnail Generation Lambda (Phase 7)
# =============================================

resource "aws_lambda_function" "generate_thumbnail" {
  filename      = "${path.module}/generate_thumbnail.zip"
  ...
  depends_on = [
    aws_iam_role_policy_attachment.lambda_basic_execution,
    aws_iam_role_policy_attachment.lambda_s3_thumbnail
  ]

  tags = {
    Name        = "${var.project_name}-generate-thumbnail"
    Environment = var.environment
    ManagedBy   = "terraform"
    Phase       = "7"
  }
...
# IAM policy for thumbnail Lambda to read/write S3
resource "aws_iam_policy" "lambda_s3_thumbnail_policy" {
  name        = "${var.project_name}-lambda-s3-thumbnail-policy"
  description = "Allow thumbnail Lambda to read source images and write thumbnails"
...
resource "aws_iam_role_policy_attachment" "lambda_s3_thumbnail" {
  role       = aws_iam_role.lambda_execution_role.name
  policy_arn = aws_iam_policy.lambda_s3_thumbnail_policy.arn
}

# Permission for S3 to invoke the thumbnail Lambda
...
# S3 bucket notification to trigger thumbnail generation
